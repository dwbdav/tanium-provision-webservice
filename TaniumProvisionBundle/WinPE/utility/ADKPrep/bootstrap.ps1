# Find and run the provision-pe.ps1 script
...
Write-Host "bootstrap.ps1 10.9.71.0"

$found = $false

try {
	Get-CimInstance -ClassName Win32_LogicalDisk -Filter "DriveType='3'" | ForEach-Object {
		if (Test-Path "$($_.DeviceID)\_t\provision-pe.ps1") {
			$found = $true
			& "$($_.DeviceID)\_t\provision-pe.ps1"
			exit $LASTEXITCODE
		}
	}

	# If we did not find \_t\provision-pe.ps1 on any already-mounted drives, we search for partitions and try and find the OS partition.
	$fixedDisks = Get-Wmiobject -Class Win32_DiskDrive -Filter "MediaType = 'Fixed hard disk media'"
	$physicalDisks = Get-PhysicalDisk | Where-Object { $fixedDisks.Index -contains $_.deviceID }

	$partitions = $physicalDisks | ForEach-Object { Get-Partition -disknumber $_.DeviceId } | Where-Object {@("System","Reserved","Recovery") -notcontains $_.Type}
	[char[]]$usableDriveLetters = "CDEFGHIJKLMNOPQRSTUVWXYZ"

	foreach ($part in $partitions) {
		$targetDriveLetter = ""
		[char[]]$inUseDriveLetters = (Get-Volume | Where-Object {$_.DriveLetter}).DriveLetter
		foreach ($letter in $usableDriveLetters) {
			if ($inUseDriveLetters -notcontains $letter) {
				$targetDriveLetter = $letter
				break
			}
		}

		if ($targetDriveLetter -eq "") {
			Write-Output "No available drive letters to use. Exiting."
			break
		}
		$part | Set-Partition -NewDriveLetter $targetDriveLetter
		if (Test-Path "$($targetDriveLetter):\_t\provision-pe.ps1") {
			$found = $true
			& "$($targetDriveLetter):\_t\provision-pe.ps1"
			exit $LASTEXITCODE
		} else {
			try {
				Remove-PartitionAccessPath -DiskNumber $part.DiskNumber -PartitionNumber $part.PartitionNumber -AccessPath "$($targetDriveLetter):\" -ErrorAction Continue
			} catch {
				Write-Output $_
			}
		}
	}
	# If not found, display an error and exit with a non-zero return code once the user clicks OK
	if (-not $found) {
		throw "Could not find provision-pe.ps1, can't continue.  Windows PE may need an additional mass storage driver."
	}
} catch {
	Write-Output "Encountered error:"
	Write-Output $_
	if ($EnableDebugPrompt) {
		try {
			$windowStyle = [System.Diagnostics.ProcessWindowStyle]::Minimized
			Start-Process "cmd.exe" -WindowStyle $windowStyle
		} catch {
			Write-Output "Could not spawn troubleshooting command prompt."
			Start-Sleep -Seconds 5
		}
	}
	$ButtonType = [System.Windows.Forms.MessageBoxButtons]::OK
	$MessageIcon = [System.Windows.Forms.MessageBoxIcon]::Stop
	$MessageBody = "Unable to find provision-pe.ps1, can't continue.  Windows PE may need an additional mass storage driver."
	$MessageTitle = "Script not found"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUVvKuwjyWitKF49NPdD7Ax+xL
# zoSggiDNMIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# CSqGSIb3DQEJBDEWBBRAPuZsY6DQhnUxeVpfN7h5ZvRydzANBgkqhkiG9w0BAQEF
# AASCAYAHUNkrA/iZ3XWIvd1Lx761O5WuroQRsK12jCdpOid0VB/nwJGo50o5WYp+
