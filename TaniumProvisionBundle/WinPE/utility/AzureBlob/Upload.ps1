...
.GUID e513c42a-0483-4e23-8ca4-eced89843ede
...
.RELEASENOTES
Version 0.9:  Initial version, uploads all contents of local cache dir
Version 1.0:  Updated to allow choosing only some of the bundles in the local cache dir, prompt before deleting 'extra' blobs, and specifying what bundles to upload at the command line
Version 1.01: Handle bug when cacheDir parameter has a trailing slash.
Version 1.1:  Fix an issue when only one bundle is specified
Version 1.2:  Include the Provision tools version in the signed script
...
.DESCRIPTION
Upload Tanium Provision cache contents to Azure Blob storage for remote imaging
.PARAMETER StorageAccountName
The Azure Storage Account name to upload to
.PARAMETER ResourceGroupName
The resource group containing the Storage Account
.PARAMETER ContainerName
The blob storage container name within the Azure Storage Account
.PARAMETER CacheDir
The local Tanium Provision cache directory
.PARAMETER Bundles
Array of bundle IDs to include in the upload
.PARAMETER Force
Do not prompt before deleting extra files on the blob storage container
.EXAMPLE
.\Upload.ps1 -storageAccountName steusprovision001 -resourceGroupName RG-Provision-Prod-001 -containerName Provision

Uploads bundle files from the default Provision Cache directory location to the Azure Storage Account
#>
[CmdletBinding()]
param (
    ...
    $CacheDir = "C:\Program Files (x86)\Tanium\Tanium Client\Tools\Provision\cache",
    [Parameter()]
    [int32[]]
    $Bundles,
    [Parameter()]
...
Write-Host "Upload.ps1 10.9.71.0"
...
function Upload-AzureFile {
    ...
    Write-Host "Uploading: $filePath ($($objFile.FullName))"
    $blob = @{
    ...
    $newBlob = Set-AzStorageBlobContent @blob
}
#endregion functions
...
$manifestFile = Get-ChildItem -Path $cacheDir -Filter "manifest.json"
$manifest = Get-Content -Path $manifestFile.FullName | ConvertFrom-Json

if ($PSBoundParameters.ContainsKey("Bundles")) {
    [object[]]$bundlesToUpload = $manifest.bundles | Where-Object {$Bundles -contains $_.id}
    if ( $bundlesToUpload.count -ne $bundles.Count ) {
        Write-Warning "$($Bundles.count) bundles indicated, but only $($bundlesToUpload.count) found."
        Write-Warning "Bundles found: $($bundlesToUpload.id -join ", "), missing bundles: $((Compare-Object -ReferenceObject $Bundles -DifferenceObject $bundlesToUpload.Id).inputobject -join ", ")"
        throw "Not all bundles were present."
    }
} else {
    if ( Get-Command -Name Out-GridView -ErrorAction SilentlyContinue ) {
        $selectedBundles = $manifest.bundles | Select-Object -Property Name,id,architecture | Out-GridView -OutputMode Multiple -Title "Please choose which bundle(s) to upload"
        [object[]]$bundlesToUpload = $manifest.bundles | Where-Object {$selectedBundles.id -contains $_.id}
    } elseif ( Get-Command -Name Out-ConsoleGridView -ErrorAction SilentlyContinue ) {
        $selectedBundles = $manifest.bundles | Select-Object -Property Name,id,architecture | Out-ConsoleGridView -OutputMode Multiple -Title "Please choose which bundle(s) to upload"
        [object[]]$bundlesToUpload = $manifest.bundles | Where-Object {$selectedBundles.id -contains $_.id}
    } else {
        Write-Warning "Could not find a gridview cmdlet to use, processing all files in ""$($cacheDir)"""
        [object[]]$bundlesToUpload = $manifest.bundles
    }
}

$objUploadManifest = [ordered]@{}
$objUploadManifest.Add("bundles", $bundlesToUpload)
$objUploadManifest.Add("globals", $manifest.globals)
$uploadManifest = $objUploadManifest | ConvertTo-Json -Depth 100

$uploadManifestFilePath = Join-Path -Path $env:temp -ChildPath "manifest.json"
$uploadManifest | Out-File $uploadManifestFilePath -Encoding utf8
$uploadManifestFile = Get-Item $uploadManifestFilePath


$hashesToUpload = $bundlesToUpload.files.meta.sha256 | Sort-Object -Unique

$filesToUpload = Get-ChildItem -Path $cacheDir -Recurse -File | Where-Object {($_.fullname -match "default") -or ($hashesToUpload -contains $_.Name)}
$filesToUpload += $manifestFile

# Upload any files not already uploaded
$current = Get-AzStorageBlob -Container $containerName -Context $context
foreach ( $objFile in $filesToUpload ) {
    $filePath = $objFile.FullName.Substring("$($cacheDir.FullName)".Length).Replace("\", "/")
    if ($filepath.Substring(0,1) -eq "/") {
        $filepath = $filepath.Substring(1)
    }
    if ( $objFile.Name -eq "manifest.json" ) {
        $objFile = $uploadManifestFile
    }
    $found = $current | Where-object { $_.Name -eq $filePath}
    $skip = $false
    if ($found) {
        $found | Add-Member -NotePropertyName 'Found' -NotePropertyValue $true
        if ($found.Length -ne $objFile.Length) {
            Write-Host "Uploading (length change): $filePath"
        } else {
    ...
    if ( -not $skip ) {
        Upload-AzureFile -objFile $objFile -filePath $filePath -context $context -containername $containerName
    }
}

$extrafiles = $current | Where-Object { -Not ($_.Found -eq $true) }
if ( $extrafiles ) {
    # Remove any extra files
    if ( -not $force ) {
        Write-Host
        Write-Host "Found extra files in the blob container:"
        $extrafiles.Name
        $title    = 'Delete extra files?'
        $question = 'Are you sure you want to delete these extra files?'
        $choices  = '&Yes', '&No'
        $decision = $Host.UI.PromptForChoice($title, $question, $choices, 1)
    }
    if ( $force -or ( $decision -eq 0 ) ) {
        $extrafiles | ForEach-Object {
            Write-Host "Removing extra: $($_.Name)"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUnxUXmCY/odGvJGZamYBlMx+c
# CDeggiDNMIIFjTCCBHWgAwIBAgIQDpsYjvnQLefv21DiCEAYWjANBgkqhkiG9w0B
...
# CSqGSIb3DQEJBDEWBBS35abzMRAXZVNSCQc0hNtEIgFz0TANBgkqhkiG9w0BAQEF
# AASCAYAZUuT77R+BjIH1xoUfZFWyDNj4R3SEb7eYka2Qe4CLnYyo4dEbI0olYJj3
# FMyDqm/tbUajHHtQVEykhQMWLPGqKXlORA1aY42zuEoGfj7tYVS454zxrjwWgtcO
# 1JnwEZALtlulJkOP9PaZY0wEiggAJMUxqc4N/2g2TghNaTs9T3wxsgu6U6cXsMt9
